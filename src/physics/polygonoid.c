...
Polygonoid CreatePolygonoid_Symmetric(int vertice_count, float radius, ColourRgba colour, size_t mass, Vector2d position, Velocity2d velocity, Acceleration2d acceleration)
{
   //
   Polygonoid newPol = {0};
   Surface2d surface = {0};
   DynamicArray* surface_vectors = GetPolygonoidSurfaceVectors_Symmetric(radius, vertice_count, position);
...
DynamicArray* GetPolygonoidSurfaceVectors_Symmetric (float radius, int vertice_count, Vector2d origin) {
