...
static Vector2d game_logical_screen_size = {0};
...
{
    int game_pixels_per_unit = game_pixels_per_unit_override;
    if (game_pixels_per_unit <= 0)
    {
        game_pixels_per_unit = (int)roundf((float)screen_height / viewport_target_game_logical_height);
        if (game_pixels_per_unit < 1)
        {
            game_pixels_per_unit = 1;
        }
    }

    int ui_pixels_per_unit = viewport_ui_pixels_per_unit_override > 0 ? viewport_ui_pixels_per_unit_override : game_pixels_per_unit;

    // 0. Resolve total screen size in game logical units.
    game_logical_screen_size = VectorScale_2d((Vector2d){(float)screen_width, (float)screen_height}, 1.0f / (float)game_pixels_per_unit);
    game_logical_screen_size.x = floorf(game_logical_screen_size.x);
    game_logical_screen_size.y = floorf(game_logical_screen_size.y);
    float total_logical_area = game_logical_screen_size.x * game_logical_screen_size.y;

    // 1. Resolve logical regions (left panel, game region, right panel).
    lpanel_origin = ZERO_VECTOR_2D;
    lpanel_end.x = floorf(lpanel_origin.x + (viewport_left_panel_ratio * game_logical_screen_size.x));
    lpanel_end.y = game_logical_screen_size.y;
    ...
    rpanel_end.x = game_logical_screen_size.x;
    rpanel_end.y = game_logical_screen_size.y;
    rpanel_origin.x = floorf(rpanel_end.x - (viewport_right_panel_ratio * game_logical_screen_size.x));
    ...
    game_region_resolution = (Vector2d){game_logical_screen_size.x - lpanel_resolution.x - rpanel_resolution.x, game_logical_screen_size.y};
    ...
    // 2. Resolve pixel-space basis vectors for each region.
    game_viewport_u = VectorScale_2d(world_u, (float)game_pixels_per_unit);
    game_viewport_v = VectorScale_2d(world_v, (float)game_pixels_per_unit);
    ...
    // 2.2 Save basis scale factors for panel coordinate conversions.
    ...
    float reconstructed_logical_area = reconstructed_logical_width * reconstructed_logical_height;
        ...
        printf("LOGICAL REGIONS --> SCREEN(AREA:%0.1f SIZE:%0.1f,%0.1f); LPANEL(AREA:%0.1f SIZE:%0.1f,%0.1f); GAME(AREA:%0.1f SIZE:%0.1f,%0.1f); RPANEL(AREA:%0.1f SIZE:%0.1f,%0.1f); RECONSTRUCTED_AREA(%0.1f);\n",
            total_logical_area, game_logical_screen_size.x, game_logical_screen_size.y,
        ...
        printf("VIEWPORT SCALE --> GAME_PX_PER_UNIT:%d UI_PX_PER_UNIT:%d TARGET_GAME_H:%.1f\n",
            game_pixels_per_unit, ui_pixels_per_unit, viewport_target_game_logical_height);
    #else
